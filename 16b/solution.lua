...
function distinct (a, b)
  a.seen['AA'] = nil
  b.seen['AA'] = nil
  for valve, _ in pairs(a.seen) do
    if b.seen[valve] then
      return false
    end
  end
  return true
...
local solutions = run(26)
print(solutions[1].score)

local best = 0
for i, aSol in ipairs(solutions) do
  for j, bSol in ipairs(solutions) do
    if j % 10000 == 1 then
      print('i/j', i, j)
    end
    if j > i  then
      if distinct(aSol, bSol) then
        best = math.max(best, aSol.score + bSol.score)
      end
    end
  end
end
